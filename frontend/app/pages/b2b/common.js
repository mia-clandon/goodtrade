...
import '../../widgets/ui-modal-b2b';
import '../../widgets/ui-commerce-offer-b2b';
import '../../widgets/ui-dropdown-menu-b2b';
...
$(function () {
    ...
    function compare_widget() {
        ...
        $.ajax({url: '/site/update-compare-widget', type: 'POST', data: {'b2b': b2b}, dataType: 'JSON'})
            .done(function(data) {
                if($('.js-compare-b2b').length > 0) {
                    let style = $('.js-compare-b2b .dropdown__item_bar-top-icon-center .dropdown__item-indicator').attr("style");
    ...
    $.validate({
        ...
        inlineErrorMessageCallback:  function($input, errorMessage, config) {
            if (errorMessage) {
                ...
                $formControl.find('.form-control__message').remove();
            }
    ...
    $inputs.each(function (index, element) {
        if ($(element).val() !== "") {
            let $icon = $("<div>", {"class" : "icon icon_delete"});
            $(element).after($icon);
