 ...
 */
export default $.widget('ui.tags', [$.ui.formResetMixin, {
    version: '1.0.0',
    options: {
        isEditable: false,
        paramName: '',
     ...
     */
    _init: function () {
        let $tags = this.element.children('.ui-tags-item');
        ...
        if (this.options.isEditable === true) {
            let $input = this.input = this._renderInput()
    ...
    addTag: function (item) {
        ...
        let $tag = this._renderTagData(item).hide();
    ...
    removeTag: function (tag) {
        ...
        this._hide(tag, this.options.hide, function () {
            $(this).remove();
            th.element.trigger('removeTag', tag.data('ui-tags-item'));
        });
    ...
    indexOfTag: function (label, value) {
        let $tags = this.element.find('.ui-tags-item');
    ...
    _inputWidth: function () {
        ...
        let width = this.buffer.text(this.inputField.val()).width();
