...
export default $.widget('ui.inputTips', $.ui.input, {
    options: {
        timer: null,
        url: '',
        type: 'POST',
    ...
    _create: function () {
        var $root = this.element,
            $tips = this.options.tips = $(this.options.selectors.tips, $root);
        this.options.tipsBody = $(this.options.selectors.tipsBody, $root);
    ...
    _setOption: function (key, val) {
        var $tips = this.option('tips'),
            ...
            mTopBef = this.option('marginTopBeforeAnim'),
            mTopAft = this.option('marginTopAfterAnim'),
            self = this;
        if (key == 'value') {
            if (val.length >= 3) {
                this._getTips();
            ...
            if (val && $input.is(':focus') && hasTips) {
                $tips.animate({marginTop: mTopAft}, {duration: duration, queue: false});
                ...
                $tips.animate({marginTop: mTopBef}, {duration: duration, queue: false});
